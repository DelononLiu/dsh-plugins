...
> **被取代（2026-08）**：引入方式已从 submodule 改为**统一 npm 安装**（见 [2026-08-23-vendoring-npm-unified](../../implemented/process/2026-08-23-vendoring-npm-unified.md)）。本 note 保留 submodule 时代的决策记录；命名/改造/License 部分仍有效。
...
- **引入方式**：git submodule 锁定 commit/tag（放 `vendored/`），不 copy 源码进 git 历史（避免丢失上游更新）。【引入方式已被 2026-08-23 note 取代】
...
- **升级**：随版本矩阵整体推进——submodule bump + 补丁层回归验证（submodule 机制已定：轻量单包本地安装/全家桶 npm 安装 + lock 锁版本）。【升级方式已被 2026-08-23 note 取代】
