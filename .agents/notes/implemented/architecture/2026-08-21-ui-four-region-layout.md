...
发行包 UI 插件（dsh-quick-nav / dsh-focus-tabs / dsh-desk / console 界面）需要统一的布局落点，否则各插件各自注入界面会混乱；皮肤中心（换肤）是否引入也需要定夺。
...
**皮肤中心否决**：用户明确"不喜欢换皮肤，功能优先"——不引入 dsh-web-ui 的 skin-center v2；dsh-desk 自定义维度收敛为**布局 + 插件组合**（vendored 全家桶时可不装 skin-center 包）。
...
- dsh-quick-nav 定位于顶部区域、dsh-focus-tabs 定位于 tab 区；console 界面入口在左侧按钮区；侧边栏走官方+better-sidebar。
