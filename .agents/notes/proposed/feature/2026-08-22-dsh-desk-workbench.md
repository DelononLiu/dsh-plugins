...
最终发布目标是**「个人 AI 工作台」发行包**：装好即是一个完整的个人 AI 工作环境，开箱即用。当前自研核心已具备（AI 对话 tabs / 快捷导航 / 管理 console+守护 / 布局 my-ui），**缺开发工具层**（侧边栏文件浏览、git、MD 预览、终端、SSH 等）与**发行包整合**。

## Decision

1. **命名**：产品/代码名 **`dsh-desk`**（个人 AI 工作台）。候选冲突：dsh-workbench / dsh-studio（npm+GitHub 已占）、dsh-mydesk（他平台名，避免混淆）。my-ui 保留为内部布局平台名。
...
4. **集成与摆位**：web profile 模板组合全部插件；my-ui 四区布局把开发工具摆到合适位置（侧边栏/内容区/顶栏）——**开箱即用**。
