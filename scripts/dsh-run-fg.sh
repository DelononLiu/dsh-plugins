...
# 解析实例名 → home|profile|port|chanId；rc=0 有效 / 1 无效（原因已打印到 stderr）。
resolve() {
  ...
  local chanId="$name"; [[ "$name" == "daemon" ]] && chanId="host-master"
  echo "$home|$prof|$port|$chanId"
}
...
run_once() {
  local home="$1" profile="$2" port="$3" chanId="$4" log="${5:-}"
  local start; start="$(date +%s)"
  echo "[$name] 启动：DSH_HOME=$home dsh --profile $profile（port ${port:-headless}） → ${log:-终端}"
  local cmd=()
  if [[ -n "$log" ]]; then
    # 输出落文件；用 tail --pid 跟随实时打到终端，直到子进程退出。
    env DSH_HOME="$home" DSH_CHANNEL_ID="$chanId" "$DSH_BIN" --profile "$profile" >"$log" 2>&1 &
    local pid=$!
    ...
    env DSH_HOME="$home" DSH_CHANNEL_ID="$chanId" "$DSH_BIN" --profile "$profile" &
    local pid=$!
    wait "$pid"; local code=$?
  fi
...
if [[ "$dry" -eq 1 ]]; then
  ...
  echo "  env DSH_HOME=$home DSH_CHANNEL_ID=$chanId $DSH_BIN --profile $profile"
...
[[ "$loop" -eq 0 ]] && { run_once "$home" "$profile" "$port" "$chanId" "$log"; exit $?; }
while :; do
  run_once "$home" "$profile" "$port" "$chanId" "$log"
  code=$?
  if [[ "$code" -eq 0 ]]; then
