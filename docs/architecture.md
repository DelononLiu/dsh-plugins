...
- [ ] **typert 接入（传输与调用分层落地）**（2026-08 定分层，见 §3「传输与调用分层」）：`typert → dsh-channel`（typert 调用帧经 channel 传输，broker 为 channel 可选后端）。**实际内核 0.1.1-rc.2 已内置 typert 运行时**（protocol/registry/loader + client-connection/api-remotes，web2 已验证 registry bundle；lock 声明已同步 rc.2）；**构建期 generator 不随内核**（npm rc.1，需单独装，TS6 环境）。**未实现**：① channel 的 typert transport 契约（`rpc.send`/`intercept`）② console/quick-nav client 面改 `ctx.remote` 消费（替换手写 HTTP /api/console/* + EventSource）③ 跨实例 `@RemoteScope` 控制指令（console→instance/daemon）④ transport 选择策略（直连 vs broker）⑤ typert forwardable events ↔ channel 三平面映射。当前跨实例仍走 relay+HTTP。
...
| 项 | 状态 | 差距 |
| --- | --- | --- |
| **typert 接入**（传输与调用分层落地，§3 已定；实际内核 0.1.1-rc.2 已内置 typert 运行时，generator 需单独装） | 待实现 | ① channel typert transport 契约（rpc.send/intercept）② console/quick-nav client 改 ctx.remote（替换手写 HTTP/EventSource）③ 跨实例 @RemoteScope 控制指令 ④ transport 选择策略（直连 vs broker）⑤ typert forwardable events ↔ channel 三平面映射 |
