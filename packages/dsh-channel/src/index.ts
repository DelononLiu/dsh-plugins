 ...
 * 识别契约；旧的无短横形态 `host<N>` 一并接受（迁移期兼容）。
 */
export function isHostAgent(agentId: string): boolean {
  return /^host-.+$/.test(agentId) || /^host\d+$/.test(agentId)
}

/**
 * 归一化守护 agent 名（`host-<id>`）：已是规范形态（`host-*`）或旧形态（`host<N>`）
 * 原样返回，其它取值补 `host-` 前缀（`'1'` → `'host-1'`，`'lab1'` → `'host-lab1'`）。
 * launch 的 host 键与守护自报名必须同形，否则 console 按键路由时判定"目标守护未注册"。
 * @param hostId - 配置里的守护 id（字符串）。
 * @returns 守护 agent 名（{@link isHostAgent} 为真者可与 launch host 键直接比对）。
 */
export function hostAgentId(hostId: string): string {
  if (isHostAgent(hostId)) return hostId
  return `host-${hostId}`
}
