...
export class ConsoleService extends TypertRemoteService {
  ...
  constructor(ctx: Context, private readonly config: Config) {
    ...
    // daemon 启动对账（异步；报告端口占用与疑似孤儿，不阻塞启动）。任务句柄留存：
    // 端口探测每实例最长 2s，消费方（注册上报/测试/UI）应 await reconcileReady()
    // 而不是猜时间窗。
    if (config.role === 'daemon') this.reconcileTask = this.reconcileInstances()
  ...
  private workerReport(): WorkerReport {
    // 守护自报名 = host-<hostId>（规范形态，id 为字符串：hostId '1' → 'host-1'）。
    // 必须与 launch 的 host 键同形，否则 console 按键路由判定"目标守护未注册"。
    const id = this.ctx.channel.instanceId ?? (this.config.hostId ? hostAgentId(this.config.hostId) : 'daemon')
  ...
  private handleWorkerRegister(report: WorkerReport): void {
    ...
    // 主机档案 upsert：**首次注册即建**（此前只更新已存在档案 → 首次注册的守护
    // 在主机表里不存在、版本列永远空）；带守护上报的发行包版本（升级编排依据）。
  ...
  /**
   * 等待启动对账完成（daemon 角色；端口探测每实例最长 2s，故异步）。
   * 非 daemon 角色立即 resolve。
   * @returns 对账完成（结果经 {@link reconcileResult} 读）。
   */
  reconcileReady(): Promise<void> {
    return this.reconcileTask ?? Promise.resolve()
  }
