...
- **测试环境 = 目录隔离 + 固定矩阵**（2026-08 定）：测试环境是**固定映射**（不靠猜，见下），sessions/settings/storages 完全隔离，不污染正式 `~/.dsh`。

  | 环境 | DSH_HOME | 启动 | 端口 | 角色 |
  | --- | --- | --- | --- | --- |
  | web2 | `~/.dsh-web2` | `DSH_HOME=~/.dsh-web2 dsh --profile web` | 3082 | 管理端 console（装全家桶 + 组装器验证） |
  | web3 | `~/.dsh-web3` | `DSH_HOME=~/.dsh-web3 dsh --profile web` | 3083 | instance 角色（`DSH_RELAY_AGENT=web3`） |
  | web4 | `~/.dsh-web4` | `DSH_HOME=~/.dsh-web4 dsh --profile web` | 3084 | instance 角色（`DSH_RELAY_AGENT=web4`） |
